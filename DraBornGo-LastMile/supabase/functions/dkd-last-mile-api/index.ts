...
const dkd_version = '0.7';
const dkd_authorityVersion = 1;
const dkd_cors = {
...
const dkd_errorText = (dkd_issue: unknown) => {
  if (dkd_issue instanceof Error) return dkd_issue.message;
  if (dkd_issue && typeof dkd_issue === 'object' && 'message' in dkd_issue) return String((dkd_issue as { message?: unknown }).message ?? 'server_error');
  return 'server_error';
};
const dkd_errorStatus = (dkd_message: string) => {
  if (/admin_required|account_not_active|player_required/i.test(dkd_message)) return 403;
  if (/not_found|expired/i.test(dkd_message)) return 404;
  if (/not_accepted|pool_full|requirements_not_met|not_pending|not_reviewable|not_approved|server_verification_required/i.test(dkd_message)) return 409;
  if (/invalid_|required/i.test(dkd_message)) return 400;
  return 500;
};
...
Deno.serve(async (dkd_request: Request) => {
  ...
  try {
    ...
    if (dkd_action === 'health') {
      return dkd_json({ dkd_ok: true, dkd_version, dkd_authority_version: dkd_authorityVersion, dkd_user_id: dkd_user.id, dkd_role });
    }
    ...
    if (dkd_action === 'bootstrap') {
      ...
      return dkd_json({ dkd_ok: true, dkd_data, dkd_version, dkd_authority_version: dkd_authorityVersion });
    }
    ...
    if (dkd_action === 'save_progress') {
      ...
      const dkd_serialized = JSON.stringify(dkd_state);
      if (dkd_serialized.length > 1_500_000) return dkd_json({ dkd_error: 'save_too_large' }, 413);
      const dkd_xp = Math.max(0, Math.min(1_000_000_000, Math.floor(Number(dkd_state.dkd_xp ?? 0) || 0)));
      const dkd_level = Math.max(1, Math.min(999, 1 + Math.floor(Math.sqrt(dkd_xp / 160))));
      const dkd_wallet = Math.max(0, Math.min(1_000_000_000, Math.floor(Number(dkd_state.dkd_wallet ?? 0) || 0)));
      const dkd_deliveries = Math.max(0, Math.min(1_000_000, Math.floor(Number(dkd_state.dkd_deliveries ?? 0) || 0)));
      const { data: dkd_saved, error: dkd_error } = await dkd_admin.rpc('dkd_lastmile_save_progress', {
        dkd_user_id: dkd_user.id,
        dkd_game_state: dkd_state,
        dkd_level,
        dkd_xp,
        dkd_wallet,
        dkd_deliveries,
      ...
      if (dkd_error) throw dkd_error;
      return dkd_json({ dkd_ok: Boolean(dkd_saved), dkd_synced_at: new Date().toISOString(), dkd_version, dkd_authority_version: dkd_authorityVersion });
    }
    ...
    if (dkd_action === 'claim_job') {
      // The RPC intentionally ignores this compatibility argument and uses dkd_server_level.
      const { data: dkd_job, error: dkd_error } = await dkd_admin.rpc('dkd_lastmile_claim_job', { dkd_user_id: dkd_user.id, dkd_level: 1 });
      if (dkd_error) throw dkd_error;
      return dkd_json({ dkd_ok: true, dkd_data: dkd_job, dkd_version, dkd_authority_version: dkd_authorityVersion });
    }

    if (dkd_action === 'submit_reward_claim') {
      const dkd_rewardId = String(dkd_body.dkd_reward_id ?? '').slice(0, 80);
      if (!/^dkd_[A-Za-z0-9_-]+$/.test(dkd_rewardId)) return dkd_json({ dkd_error: 'invalid_reward_id' }, 400);
      const dkd_submittedScore = Math.max(0, Math.min(100_000, Math.floor(Number(dkd_body.dkd_final_score ?? 0) || 0)));
      const { data: dkd_claim, error: dkd_error } = await dkd_admin.rpc('dkd_lastmile_submit_reward_claim', {
        dkd_user_id: dkd_user.id,
        dkd_reward_id: dkd_rewardId,
        dkd_final_score: dkd_submittedScore,
      });
      if (dkd_error) throw dkd_error;
      return dkd_json({ dkd_ok: true, dkd_data: dkd_claim, dkd_version, dkd_authority_version: dkd_authorityVersion });
    }

    if (dkd_action === 'admin_reward_queue') {
      if (dkd_role !== 'admin') return dkd_json({ dkd_error: 'admin_required' }, 403);
      const { data: dkd_queue, error: dkd_error } = await dkd_admin.rpc('dkd_lastmile_reward_queue', { dkd_admin_user_id: dkd_user.id });
      if (dkd_error) throw dkd_error;
      return dkd_json({ dkd_ok: true, dkd_data: dkd_queue, dkd_version });
    }

    if (dkd_action === 'review_reward_claim') {
      if (dkd_role !== 'admin') return dkd_json({ dkd_error: 'admin_required' }, 403);
      const dkd_claimId = String(dkd_body.dkd_claim_id ?? '');
      if (!/^[0-9a-f-]{36}$/i.test(dkd_claimId)) return dkd_json({ dkd_error: 'invalid_claim_id' }, 400);
      const dkd_decision = String(dkd_body.dkd_decision ?? '').toLowerCase();
      if (!['approved', 'rejected', 'fulfilled'].includes(dkd_decision)) return dkd_json({ dkd_error: 'invalid_decision' }, 400);
      const dkd_note = String(dkd_body.dkd_note ?? '').trim().slice(0, 1000);
      if (dkd_note.length < 3) return dkd_json({ dkd_error: 'review_note_required' }, 400);
      const { data: dkd_claim, error: dkd_error } = await dkd_admin.rpc('dkd_lastmile_review_reward_claim', {
        dkd_admin_user_id: dkd_user.id,
        dkd_claim_id: dkd_claimId,
        dkd_decision,
        dkd_note,
      });
      if (dkd_error) throw dkd_error;
      return dkd_json({ dkd_ok: true, dkd_data: dkd_claim, dkd_version });
    }
    ...
    if (['accept_job', 'cancel_job', 'complete_job'].includes(dkd_action)) {
      ...
      if (JSON.stringify(dkd_metrics).length > 32_000) return dkd_json({ dkd_error: 'metrics_too_large' }, 413);
    ...
    const dkd_message = dkd_errorText(dkd_issue);
    console.error('dkd-last-mile-api', dkd_message);
    return dkd_json({ dkd_error: dkd_message }, dkd_errorStatus(dkd_message));
  }
});