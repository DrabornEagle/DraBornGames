...
export function dkd_defaultState() {
  return { dkd_schema: 1, dkd_profile: null, dkd_wallet: 900, dkd_xp: 0, dkd_master: 0, dkd_deliveries: 0, dkd_onTime: 0, dkd_damageFree: 0, dkd_storm: 0, dkd_night: 0, dkd_distance: 0, dkd_ratingTotal: 0, dkd_vipTrust: 0, dkd_tokens: 0, dkd_vip: null, dkd_chapter: 0, dkd_prize: 'dkd_laptop', dkd_season: 'dkd_season01', dkd_tutorial: false, dkd_fullCareer: false, dkd_equipped: 'dkd_city50', dkd_owned: ['dkd_city50'], dkd_upgrades: { dkd_engine: 0, dkd_grip: 0, dkd_bag: 0 }, dkd_fleet: { dkd_city50: { dkd_fuel: 100, dkd_condition: 100 } }, dkd_garage: 0, dkd_brand: { dkd_color: '#e4ff5e', dkd_logo: 'eagle', dkd_uniform: '#233d44', dkd_plate: '06 SON 01' }, dkd_cosmetics: [], dkd_wearing: [], dkd_settings: { dkd_quality: 'balanced', dkd_motion: true, dkd_music: .55, dkd_effects: .8, dkd_assist: true, dkd_haptics: true, dkd_camera: 'chase', dkd_steering: .7 }, dkd_reviews: [], dkd_transactions: [], dkd_messages: [], dkd_scores: [], dkd_ghost: null, dkd_black: 0, dkd_daily: { dkd_date: '', dkd_deliveries: 0, dkd_clean: 0, dkd_vip: 0, dkd_rain: 0, dkd_distance: 0, dkd_rated: 0, dkd_claimed: false }, dkd_streak: { dkd_last: '', dkd_days: 0, dkd_claimed: [] }, dkd_training: false, dkd_completedRuns: [], dkd_lastRun: null };
}

export function dkd_historyLabel(dkd_label) {
  const dkd_names = [["City 50", "Şehir 50"], ["E / Courier", "Elektrikli Kurye"], ["Urban 125", "Şehirli 125"], ["Maxi / GT", "Uzun Yol 250"], ["Street / 400", "Sokak 400"], ["Terra / ADV", "Gezgin 650"], ["Apex / RR", "Zirve 600"], ["Metro / EV", "Kent Elektrikli"], ["Cargo / XL", "Kargo Büyük"], ["Storm / Unit", "Fırtına Özel"], ["Nova Residence", "Yıldız Konutları"], ["Atlas Plaza", "Atlas İş Merkezi"], ["Orbit Teknoloji", "Yörünge Teknoloji"], ["Central Otel", "Merkez Oteli"], ["Frame Stüdyo", "Kare Stüdyosu"], ["Pulse Etkinlik", "Ritim Etkinlik"], ["North Depo", "Kuzey Deposu"], ["Luna Restoran", "Ay Restoranı"], ["THE LAST DELIVERY", "SON TESLİMAT"], ["Yerel Ghost geçildi", "Yerel Hayalet Kurye geçildi"]];
  return dkd_names.reduce((dkd_text, dkd_pair) => dkd_text.split(dkd_pair[0]).join(dkd_pair[1]), dkd_label);
}
// Saves are explicitly untrusted. They are local deneme progression, never prize evidence.
export function dkd_restoreState(dkd_raw) {
  ...
  dkd_state.dkd_brand.dkd_plate = String(dkd_raw.dkd_brand?.dkd_plate || '06 SON 01').replace(/[^A-Za-z0-9 ]/g, '').slice(0, 12);
  ...
  for (const dkd_key of ['dkd_assist', 'dkd_haptics', 'dkd_motion']) if (typeof dkd_settings[dkd_key] === 'boolean') dkd_state.dkd_settings[dkd_key] = dkd_settings[dkd_key];
  ...
  dkd_state.dkd_transactions = dkd_items('dkd_transactions').filter(dkd_item => Number.isFinite(dkd_item.dkd_amount)).map(dkd_item => ({ dkd_label: dkd_historyLabel(dkd_text(dkd_item.dkd_label)), dkd_amount: dkd_clamp(dkd_item.dkd_amount, -99999999, 99999999), dkd_at: dkd_number(dkd_item.dkd_at) }));
...
export function dkd_createRun(dkd_state, dkd_graph, dkd_order, dkd_mode = 'safe', dkd_now = Date.now()) {
  if (dkd_order.dkd_type !== 'tutorial' && !dkd_state.dkd_fullCareer) throw new Error('Önce ücretsiz deneme kariyerini aç.');
  if (dkd_order.dkd_type === 'final' && !dkd_finalUnlocked(dkd_state)) throw new Error('Final Görevi koşulları tamamlanmadı.');
...
export function dkd_stepRun(dkd_run, dkd_graph, dkd_input, dkd_delta) {
  ...
  while (dkd_run.dkd_routeIndex < dkd_run.dkd_route.dkd_nodes.length && dkd_distance(dkd_run.dkd_position, dkd_graph.dkd_points[dkd_run.dkd_route.dkd_nodes[dkd_run.dkd_routeIndex]]) < (dkd_run.dkd_assist ? 6 : 13)) { const dkd_node = dkd_run.dkd_route.dkd_nodes[dkd_run.dkd_routeIndex]; dkd_run.dkd_checkpoints.push({ dkd_node, dkd_time: dkd_run.dkd_elapsed }); if (dkd_run.dkd_pendingStops?.[0] === dkd_node) { dkd_run.dkd_pendingStops.shift(); if (dkd_run.dkd_order.dkd_type === 'final') dkd_run.dkd_notifications.push({ dkd_title: 'KONTROL NOKTASI', dkd_text: `${3 - dkd_run.dkd_pendingStops.length} / 3 kontrol noktası geçildi.` }); } dkd_run.dkd_routeIndex++; }
  ...
  if (dkd_run.dkd_order.dkd_type === 'final' && dkd_run.dkd_elapsed > 48 && dkd_run.dkd_elapsed < 75) { dkd_run.dkd_signal = false; if (!dkd_run.dkd_signalMessaged) { dkd_run.dkd_signalMessaged = true; dkd_run.dkd_notifications.push({ dkd_title: 'SİNYAL KESİLDİ', dkd_text: 'Müşteri: Son kayıtlı güzergâhı takip et. Servis girişinde bekliyorum.' }); } } else dkd_run.dkd_signal = true;
...
export function dkd_completeRun(dkd_state, dkd_run, dkd_graph) {
  ...
  if (dkd_run.dkd_order.dkd_type === 'final') {
    if (dkd_audit.dkd_status === 'LOCAL_CHECK_PASSED' && dkd_state.dkd_ghost?.dkd_season === dkd_state.dkd_season && dkd_run.dkd_elapsed < dkd_state.dkd_ghost.dkd_time) { dkd_run.dkd_ghostBeaten = true; dkd_transaction(dkd_state, 350, 'Yerel Hayalet Kurye geçildi / rota bonusu'); }
...
export function dkd_dailyRequirements(dkd_state) { return [{ dkd_name: '5 hasarsız teslimat', dkd_have: dkd_state.dkd_daily.dkd_clean, dkd_need: 5 }, { dkd_name: '3 Özel Müşteri teslimatı', dkd_have: dkd_state.dkd_daily.dkd_vip, dkd_need: 3 }, { dkd_name: '12 km sürüş', dkd_have: dkd_state.dkd_daily.dkd_distance / 1000, dkd_need: 12 }, { dkd_name: 'Yağmurda 2 teslimat', dkd_have: dkd_state.dkd_daily.dkd_rain, dkd_need: 2 }, { dkd_name: '4.8+ puanlı teslimat', dkd_have: dkd_state.dkd_daily.dkd_rated, dkd_need: 1 }]; }
...
export function dkd_npcReply(dkd_state, dkd_customerId, dkd_text) {
  ...
  if (/ödül|telefon numara|gerçek|iphone|kazan/.test(dkd_normal)) return 'Ben kurgusal bir oyun karakteriyim. Bu deneme sürümünde fiziksel ödül ve gerçek görüşme yok. Kariyerindeki görevlere yardımcı olabilirim.';
