...
function dkd_v061BuildStarterModel(dkd_bytes) {
  ...
  for (let dkd_meshIndex = 0; dkd_meshIndex < dkd_meshCount; dkd_meshIndex += 1) {
    ...
    for (let dkd_vertex = 0; dkd_vertex < dkd_vertexCount; dkd_vertex += 1) {
      for (let dkd_axis = 0; dkd_axis < 3; dkd_axis += 1) {
        ...
        dkd_positions[dkd_vertex * 3 + dkd_axis] =
          dkd_min[dkd_axis] + (dkd_max[dkd_axis] - dkd_min[dkd_axis]) * dkd_quantized / 255;
      }
    }
    ...
    for (let dkd_index = 0; dkd_index < dkd_indices.length; dkd_index += 1) {
      ...
      if (dkd_previousIndex < 0 || dkd_previousIndex >= dkd_vertexCount) {
        throw new Error(`v0.6.1 mesh ${dkd_meshIndex + 1} indeks verisi geçersiz.`);
      }
    ...
    const dkd_material = new dkd_three.MeshStandardMaterial({
      ...
      roughness: dkd_clamp(dkd_roughness, .08, 1),
      transparent: dkd_alpha < 250,
      opacity: dkd_alpha / 255,
      depthWrite: dkd_alpha > 180,
  ...
  if (dkd_totalVertices !== dkd_v061ModelVertexCount || dkd_totalFaces !== dkd_v061ModelFaceCount) {
    throw new Error('v0.6.1 model geometri toplamı geçersiz.');
  }
  return dkd_group;
}
...
dkd_Scene.prototype.dkd_buildBike = function dkd_v061BuildBike(dkd_kind = 'scooter') {
  ...
  const dkd_isStarter = dkd_kind === 'scooter'
    && this.dkd_state?.dkd_equipped === 'dkd_city50'
    && dkd_fallback;
  ...
  dkd_v061ReadModelBytes()
    .then(dkd_bytes => dkd_v061BuildStarterModel(dkd_bytes))
    .then(dkd_model => {
      if (this.dkd_v061ModelLoadToken !== dkd_token
        || this.dkd_state?.dkd_equipped !== 'dkd_city50'
        || this.dkd_bike !== dkd_fallback) {
        dkd_v061DisposeObject(dkd_model);
        return;
      }
      ...
      if (this.dkd_ghost) {
        const dkd_ghostWasVisible = this.dkd_ghost.visible === true;
        const dkd_oldGhost = this.dkd_ghost;
        this.dkd_scene.remove(dkd_oldGhost);
        ...
        dkd_ghost.traverse(dkd_object => {
          ...
          dkd_object.material = new dkd_three.MeshBasicMaterial({
            color: '#65d5d0',
            transparent: true,
            opacity: .23,
            depthWrite: false,
        ...
        dkd_ghost.visible = dkd_ghostWasVisible;
        this.dkd_ghost = dkd_ghost;
        this.dkd_scene.add(dkd_ghost);
      }
    })
...
function dkd_v061PortraitInfo(dkd_source) {
  ...
  const dkd_comma = dkd_source.indexOf(',');
  if (dkd_comma < 0) return null;
  const dkd_meta = dkd_source.slice(0, dkd_comma).toLowerCase();
  const dkd_payload = dkd_source.slice(dkd_comma + 1);
  if (!dkd_meta.includes(';base64') || dkd_payload.length < 1200) return null;
  try {
    const dkd_bytes = atob(dkd_payload);
    if (dkd_bytes.length < 2800) return null;
    if (dkd_meta.includes('jpeg') || dkd_meta.includes('jpg')) {
      if (dkd_bytes.charCodeAt(0) !== 0xff || dkd_bytes.charCodeAt(1) !== 0xd8) return null;
      if (dkd_bytes.charCodeAt(dkd_bytes.length - 2) !== 0xff || dkd_bytes.charCodeAt(dkd_bytes.length - 1) !== 0xd9) return null;
      return { dkd_source, dkd_bytes: dkd_bytes.length };
    }
    if (dkd_meta.includes('png')) {
      if (dkd_bytes.slice(1, 4) !== 'PNG') return null;
      return { dkd_source, dkd_bytes: dkd_bytes.length };
    }
    if (dkd_meta.includes('webp')) {
      if (dkd_bytes.slice(0, 4) !== 'RIFF' || dkd_bytes.slice(8, 12) !== 'WEBP') return null;
      return { dkd_source, dkd_bytes: dkd_bytes.length };
    }
  } catch {}
  return null;
...
const dkd_v061SafePortraitPool = Array.from(new Set([
  ...dkd_v06CustomerPortraitPool,
  ...dkd_v061KnownPortraits,
])).filter(dkd_source => dkd_v061PortraitInfo(dkd_source));

if (dkd_v061SafePortraitPool.length < 8) {
  dkd_v061SafePortraitPool.splice(0, dkd_v061SafePortraitPool.length, ...dkd_v061KnownPortraits);
}
...
dkd_v06AssignCustomerPortraits = function dkd_v061AssignCustomerPortraits(dkd_game) {
  ...
  const dkd_poolSize = dkd_v061SafePortraitPool.length;
  if (!dkd_poolSize || !dkd_orders.length) return;

  const dkd_used = new Set();
  const dkd_stableOrders = dkd_orders
    .map((dkd_order, dkd_index) => ({
      dkd_order,
      dkd_index,
      dkd_key: dkd_v06CustomerOrderKey(dkd_order, dkd_index),
    }))
...
window.dkd_v061AvatarError = function dkd_v061AvatarError(dkd_image, dkd_startIndex) {
  if (!dkd_image) return;
  const dkd_attempt = Number(dkd_image.dataset.dkdFallbackAttempt || 0) + 1;
  dkd_image.dataset.dkdFallbackAttempt = String(dkd_attempt);
  if (dkd_attempt <= dkd_v061SafePortraitPool.length) {
    const dkd_next = (Number(dkd_startIndex || 0) + dkd_attempt) % dkd_v061SafePortraitPool.length;
    dkd_image.src = dkd_v061SafePortraitPool[dkd_next];
    return;
  }
  dkd_image.onerror = null;
  dkd_image.src = dkd_v06AvatarSelin;
};

dkd_v06CustomerOrderAvatar = function dkd_v061CustomerOrderAvatar(dkd_order, dkd_large = false) {
  const dkd_poolSize = dkd_v061SafePortraitPool.length;
  const dkd_indexRaw = Number(dkd_order?.dkd_customerPortraitIndex);
  const dkd_index = Number.isInteger(dkd_indexRaw) && dkd_poolSize
    ? ((dkd_indexRaw % dkd_poolSize) + dkd_poolSize) % dkd_poolSize
    : 0;
  const dkd_source = dkd_v061SafePortraitPool[dkd_index] || dkd_v06AvatarSelin;
  const dkd_name = dkd_escape(dkd_order?.dkd_cloudCustomerName || 'Müşteri');
  return `<img class="dkd-avatar${dkd_large ? ' dkd-large' : ''}" src="${dkd_source}" alt="" aria-label="${dkd_name} müşteri profil fotoğrafı" decoding="async" onerror="window.dkd_v061AvatarError(this,${dkd_index})"/>`;
};

function dkd_v061InstallStyles() {
  if (document.getElementById('dkd-v061-style')) return;
  const dkd_style = document.createElement('style');
  dkd_style.id = 'dkd-v061-style';
  dkd_style.textContent = `
    ...
    .dkd-avatar.dkd-large{object-position:center center!important}
  `;
...
function dkd_v061CreateHomeMusicBuffer(dkd_context) {
  const dkd_bpm = 72;
  const dkd_beats = 16;
  ...
  const dkd_roots = [55, 65.41, 73.42, 49];
  const dkd_notes = [1.5, 2, 2.5, 3, 2, 1.5, 3, 2.5];
  ...
  for (let dkd_index = 0; dkd_index < dkd_length; dkd_index += 1) {
    ...
    const dkd_breathe = .72 + .28 * Math.sin(dkd_tau * dkd_time / dkd_seconds);
    let dkd_sample = Math.sin(dkd_tau * dkd_root * dkd_time) * .075 * dkd_breathe;
    dkd_sample += Math.sin(dkd_tau * dkd_root * 1.5 * dkd_time) * .032 * dkd_breathe;
    dkd_sample += Math.sin(dkd_tau * dkd_root * .5 * dkd_time) * .028;

    const dkd_pluckFrequency = dkd_root * dkd_notes[dkd_eighthIndex % dkd_notes.length];
    const dkd_pluckEnvelope = Math.exp(-dkd_eighthPhase * 4.7);
    dkd_sample += Math.sin(dkd_tau * dkd_pluckFrequency * dkd_time) * dkd_pluckEnvelope * .038;
    dkd_sample += Math.sin(dkd_tau * dkd_pluckFrequency * 2 * dkd_time) * dkd_pluckEnvelope * .008;

    if (dkd_beatIndex % 4 === 0) {
      dkd_sample += Math.sin(dkd_tau * 42 * dkd_time) * Math.exp(-dkd_beatPhase * 9) * .026;
    }
    dkd_data[dkd_index] = dkd_sample / (1 + Math.abs(dkd_sample) * .55);
  }
  return dkd_buffer;
}

function dkd_v061StopHomeMusic(dkd_audio) {
  if (!dkd_audio?.dkd_v061HomeSource) return;
  dkd_v05StopSource(dkd_audio.dkd_v061HomeSource);
  dkd_audio.dkd_v061HomeSource = null;
}

function dkd_v061ApplyHomeMusic(dkd_game, dkd_page) {
  const dkd_audio = dkd_game?.dkd_audio;
  if (!dkd_audio) return;
  if (dkd_page !== 'home') {
    if (dkd_audio.dkd_v061HomeSource) {
      dkd_v061StopHomeMusic(dkd_audio);
      if (!dkd_game?.dkd_run?.dkd_active) {
        dkd_v05SwitchTrack(dkd_audio, dkd_audio.dkd_v05MenuTrack ?? dkd_audio.dkd_track ?? 0, 'menu');
  ...
  if (!dkd_audio.dkd_v061HomeBuffer) dkd_audio.dkd_v061HomeBuffer = dkd_v061CreateHomeMusicBuffer(dkd_audio.dkd_context);
  if (!dkd_audio.dkd_v061HomeSource) {
    ...
    dkd_source.buffer = dkd_audio.dkd_v061HomeBuffer;
  ...
  dkd_audio.dkd_v04MenuMusic.gain.setTargetAtTime(dkd_setting * .78, dkd_audio.dkd_context.currentTime, .22);
  dkd_audio.dkd_v04DriveMusic.gain.setTargetAtTime(0, dkd_audio.dkd_context.currentTime, .16);
  dkd_audio.dkd_v04Mode = 'v061-calm-home';
}

dkd_Game.prototype.dkd_render = function dkd_v061Render(dkd_page) {
  const dkd_result = dkd_v061Previous.dkd_render.call(this, dkd_page);
  dkd_v061ApplyHomeMusic(this, dkd_page);
  return dkd_result;
};

dkd_Game.prototype.dkd_view_settings = function dkd_v061Settings() {
  return String(dkd_v061Previous.dkd_view_settings.call(this))
    .replace(/v0\.5\s*·\s*Çevrimiçi kariyer/g, 'v0.6.1 · Çevrimiçi kariyer')
    .replace(/v0\.6(?!\.\d)/g, 'v0.6.1');
};

window.dkd_lastMileRelease = 'v0.6.1';
window.dkd_lastMileCustomerPortraitCount = dkd_v061SafePortraitPool.length;
window.dkd_lastMileStarterModel = {
  dkd_name: 'Yamaha SoulGT 125',
  dkd_rider: 'Quaternius Worker Male',
  dkd_meshes: dkd_v061ModelMeshCount,
  dkd_vertices: dkd_v061ModelVertexCount,
  dkd_faces: dkd_v061ModelFaceCount,
};
