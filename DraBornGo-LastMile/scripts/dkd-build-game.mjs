...
const dkd_modules = await Promise.all(dkd_sources.map(async dkd_file => {
  ...
  // v0.2 paketi kaynak kayıt uyumluluğunu bozmadan ekrandaki sürüm etiketini yükseltir.
  if (dkd_file === 'dkd-data.mjs') dkd_source = dkd_source.replace("export const dkd_version = 'v0.101';", "export const dkd_version = 'v0.2';");
  return dkd_source
    .replace(/^import .*?;\s*$/gm, '')
...
console.log(`DKD oyun paketi hazır: ${(Buffer.byteLength(dkd_html) / 1024 / 1024).toFixed(2)} MB. v0.2 · Harici CDN veya çalışma anında ağ isteği yok.`);
