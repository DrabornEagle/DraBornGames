...
// Historical v0.4/v0.5 compatibility layers remain bundled beneath the authoritative v0.6 runtime.
const dkd_sources = [
  'data/dkd-roads.mjs',
  'dkd-data.mjs',
  'dkd-core.mjs',
...
const dkd_modules = await Promise.all(dkd_sources.map(async dkd_file => {
  ...
  if (dkd_file === 'dkd-data.mjs') {
    dkd_source = dkd_source.replace("export const dkd_version = 'v0.101';", "export const dkd_version = 'v0.6';");
    // Legacy synthetic leaderboard rows stay in historical source but never enter the v0.6 game bundle.
...
console.log(`DKD oyun paketi hazır: ${(Buffer.byteLength(dkd_html) / 1024 / 1024).toFixed(2)} MB. v0.6 · gerçek sezon · 47 farklı müşteri profil fotoğrafı · admin seviye 50 · plaka kaydı · sunucu final doğrulaması · MP3 ses ayrımı · Supabase native köprü.`);
