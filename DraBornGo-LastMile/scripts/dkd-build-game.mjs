...
const dkd_modules = await Promise.all(dkd_sources.map(async dkd_file => {
  ...
  if (dkd_file === 'dkd-data.mjs') {
    dkd_source = dkd_source.replace("export const dkd_version = 'v0.101';", "export const dkd_version = 'v0.4';");
    // Legacy synthetic leaderboard rows stay in historical source but never enter the v0.4 game bundle.
...
console.log(`DKD oyun paketi hazır: ${(Buffer.byteLength(dkd_html) / 1024 / 1024).toFixed(2)} MB. v0.4 · Supabase bağlantısı native köprü üzerinden.`);
