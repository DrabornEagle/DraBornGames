...
async function dkd_patchFile(dkd_relative, dkd_patch) {
  ...
  if (dkd_after === dkd_before) {
    console.log(`DKD v0.6.1 zaten güncel: ${dkd_relative}`);
    return;
  }
  await dkd_fs.writeFile(dkd_file, dkd_after);
  console.log(`DKD v0.6.1 güncellendi: ${dkd_relative}`);
}
...
await dkd_patchFile('App.tsx', dkd_source => {
  ...
  if (!dkd_next.includes('const dkd_publicWebBase')) {
    dkd_next = dkd_next.replace(
  ...
  if (!dkd_next.includes("dkd_message.dkd_type === 'open-url'")) {
    dkd_next = dkd_next.replace(
  ...
  if (!dkd_next.includes('dkd_publicWebBase') || !dkd_next.includes("auth-delete-account")) throw new Error('App.tsx v0.6.1 patch doğrulanamadı.');
  return dkd_next;
});

await dkd_patchFile('scripts/dkd-build-game.mjs', dkd_source => {
  let dkd_next = dkd_source;
  if (!dkd_next.includes("models/v061/dkd-v061-model-0.mjs")) {
    dkd_next = dkd_next.replace(
  ...
  dkd_next = dkd_next
    .replace("export const dkd_version = 'v0.101';\", \"export const dkd_version = 'v0.6';", "export const dkd_version = 'v0.101';\", \"export const dkd_version = 'v0.6.1';")
    .replace(/SON KİLOMETRE · v0\.6</g, 'SON KİLOMETRE · v0.6.1<')
    .replace(/MB\. v0\.6 · gerçek sezon/g, 'MB. v0.6.1 · gerçek sezon');
