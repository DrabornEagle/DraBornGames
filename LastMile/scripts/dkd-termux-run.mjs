...
const dkd_app = JSON.parse(dkd_fs.readFileSync('app.json', 'utf8'));
const dkd_version = String(dkd_app?.expo?.version || '0.7.2');
...
if (dkd_fs.existsSync(dkd_lock)) {
  ...
  if (Number.isInteger(dkd_pid) && dkd_pid > 1) {
    try { process.kill(dkd_pid, 0); dkd_running = true; } catch {}
  }
  if (dkd_running) {
    console.error('SON KİLOMETRE başlatıcısı zaten açık. Önce mevcut oturumda Ctrl+C kullan.');
    process.exit(1);
  }
...
const dkd_killGroup = (dkd_child, dkd_signal = 'SIGTERM') => {
  if (!dkd_child || dkd_child.exitCode !== null) return;
  try { process.kill(-dkd_child.pid, dkd_signal); } catch {
    try { dkd_child.kill(dkd_signal); } catch {}
  }
};
const dkd_wait = dkd_child => new Promise(dkd_resolve => {
  dkd_child.once('error', () => dkd_resolve(1));
  dkd_child.once('exit', dkd_code => dkd_resolve(dkd_code ?? 1));
});

async function dkd_syncOnce() {
  dkd_sync = dkd_spawn('bash', ['scripts/dkd-sync.sh', 'once'], { cwd: dkd_root, stdio: 'inherit', detached: true });
  const dkd_code = await dkd_wait(dkd_sync);
  dkd_sync = null;
  return dkd_code;
}

async function dkd_dependencies() {
  const dkd_hash = dkd_createHash('sha256')
    .update(dkd_fs.readFileSync('package-lock.json'))
  ...
  const dkd_code = await dkd_wait(dkd_install);
  dkd_install = null;
...
async function dkd_buildSharedGame() {
  console.log(`LastMile v${dkd_version} ortak Android/web paketi ve iki MP3 doğrulanıyor.`);
  dkd_build = dkd_spawn('npm', ['run', 'build:game'], { stdio: 'inherit', detached: true });
  const dkd_code = await dkd_wait(dkd_build);
  dkd_build = null;
  if (dkd_code !== 0) throw new Error('Ortak oyun paketi üretilemedi veya MP3 doğrulaması başarısız oldu.');
}

function dkd_startMetro() {
  if (dkd_stopping) return;
  console.log(`\nSON KİLOMETRE v${dkd_version} / Expo Go 57.0.9 / SDK 57`);
  console.log('Expo Go içinde şu adresi aç: exp://127.0.0.1:8081');
  console.log('GitHub main her 30 saniyede kontrol edilir. Çıkış: Ctrl+C.\n');
  dkd_metro = dkd_spawn(
    process.execPath,
    ['node_modules/expo/bin/cli', 'start', '--go', '--localhost', '--port', '8081', '--clear'],
    {
      stdio: 'inherit',
      detached: true,
      env: { ...process.env, REACT_NATIVE_PACKAGER_HOSTNAME: '127.0.0.1', CI: '1' },
    },
  );
  const dkd_child = dkd_metro;
  dkd_child.once('error', dkd_error => {
    console.error(dkd_error.message);
    void dkd_stop(1);
  });
  dkd_child.once('exit', dkd_code => {
    if (dkd_metro === dkd_child && !dkd_busy && !dkd_stopping) void dkd_stop(dkd_code || 0);
  });
}
...
async function dkd_update() {
  ...
  try {
    ...
    if (dkd_syncCode === 0 && dkd_before !== dkd_git('rev-parse', 'HEAD')) {
      ...
      await dkd_stopMetro();
      await dkd_dependencies();
      await dkd_buildSharedGame();
      dkd_startMetro();
    }
  } catch (dkd_error) {
    console.error(dkd_error.message);
    if (!dkd_metro) await dkd_stop(1);
  } finally {
    dkd_busy = false;
  }
}
...
async function dkd_stop(dkd_code = 0) {
  if (dkd_stopping) return;
  dkd_stopping = true;
  clearInterval(dkd_poll);
  dkd_killGroup(dkd_sync);
  dkd_killGroup(dkd_install);
  dkd_killGroup(dkd_build);
  await dkd_stopMetro();
  try {
    if (dkd_fs.readFileSync(dkd_lock, 'utf8') === String(process.pid)) dkd_fs.unlinkSync(dkd_lock);
  } catch {}
...
try {
  ...
  dkd_busy = false;
  dkd_startMetro();
  dkd_poll = setInterval(() => void dkd_update(), 30000);
} catch (dkd_error) {
  console.error(dkd_error.message);
  await dkd_stop(1);
}
